  ...
  <div class="edit-address">
      ...
      <div class="edit-content" v-if="adsData">
          <div class="form">
           ...
           <FormItem :title="'收货地址'">
               <p slot="center" @click="showErea">{{adsData.district.districts}}</p>
...
methods: {
    ...
    getAds(){
        this.adsData = this.$route.params.ads
        this.currentIndex = this.adsData.sex
    },
    saveAddress(){
        // 判断地址是否为空
       ...
       if(!this.adsData.district.districts.length ||!this.adsData.accept_name.length){
        ...
        let addresses = this.$route.params.addresses
       let newAddress= addresses.map(item=>{
           if(item.id===this.adsData.id){
               return this.adsData
           }else{
               return item
           }
        })
    this.SET_ADDRESSES(newAddress)
    this.back()
    ...
    changeDistrict(val){
            ...
            this.adsData.district.districts = val.map(item=>{
            return item.name
        }).join('')
        this.show = false
    },
